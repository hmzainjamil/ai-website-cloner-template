...
> **AI website cloner template — reverse-engineer any site with Claude Code**

![Status](https://img.shields.io/badge/status-active-brightgreen?style=flat)
![License](https://img.shields.io/badge/license-MIT-blue?style=flat)
![Claude Code](https://img.shields.io/badge/Claude_Code-Skill-FF6B35?style=flat)
![Stars](https://img.shields.io/github/stars/hmzainjamil/ai-website-cloner-template?style=flat)
![Last Commit](https://img.shields.io/github/last-commit/hmzainjamil/ai-website-cloner-template?style=flat)
...
| Concept | Description |
|---|---|
| **Website** | Core primitive of ai website cloner template — fundamental building block |
| **Cloner** | Execution layer handling cloner operations and state |
| **Ai** | Integration point connecting AI to external services |
| **Claude** | Configuration schema for claude behavior and routing |
| **Reverse-Engineer** | Optimization layer reducing reverse-engineer overhead and latency |
...
/skills load ai-website-cloner-template

# Check skill status
/skills list | grep ai

# Clone and explore locally
gh repo clone hmzainjamil/ai-website-cloner-template
cd ai-website-cloner-template

# Run with Claude Code (invoke skill directly)
claude -p "use ai website cloner template skill to handle my task"
...
- **Digital agencies** — automate website workflows, cut delivery time 10×
- **SaaS founders** — plug into existing Claude Code setup, zero infra overhead
- **Freelancers** — use as force-multiplier for client work in cloner + AI
...
cp -r . ~/.claude/skills/ai-website-cloner-template/

# Option 3: Via gh CLI
gh repo clone hmzainjamil/ai-website-cloner-template ~/.claude/skills/ai-website-cloner-template
```
...
response = client.messages.create(
    ...
    max_tokens=4096,
    system="You are using the ai-website-cloner-template skill. AI website cloner template — reverse-engineer any site with Claude Code",
...
mae run "ai website cloner template - execute primary workflow"

# Blast across multiple models
tcc blast "task1 for website" "task2 for cloner" "task3 for AI"
```
...
```yaml
# .claude/skills/ai-website-cloner-template/config.yaml
skill:
...
User Intent
    │
    ▼
Skill Router ──► Trigger Match ──► ai-website-cloner-template
                                       │
                    ┌──────────────────┼──────────────────┐
                    ▼                  ▼                   ▼
...
```json
{
  "nodes": [
    {
      "name": "ai-website-cloner-template Trigger",
    ...
    },
    {
      ...
      "parameters": {
        ...
        "body": {{"skill": "ai-website-cloner-template", "input": "{{$json.body}}"}}
      }
    }
  ]
}
...
def run_skill(task: str) -> str:
    result = subprocess.run(
        ['claude', '-p', f'using ai-website-cloner-template skill: {task}'],
        capture_output=True, text=True
    )
    return result.stdout

output = run_skill("website analysis for my project")
print(output)
```
...
1. Fork this repo
2. Create feature branch: `git checkout -b feature/website-improvement`
3. Add/update `SKILL.md` with new capabilities
4. Test: `claude -p "test ai-website-cloner-template skill"`
